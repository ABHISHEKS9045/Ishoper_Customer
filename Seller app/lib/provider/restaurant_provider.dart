...
class RestaurantProvider extends ChangeNotifier {
  ...
  Future addProductImage( pickedLogo,String type, Function callback) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse response = await restaurantRepo.addImage( File(pickedLogo.path) , type);
    if(response.response != null && response.response.statusCode == 200) {
      _isLoading = false;
   ...
   submitProduct(BuildContext context, Product product, AddProductModel addProduct, List<String> productImages, String thumbnail, String metaImage, String token, bool isAdd,bool isActiveColor,
      from_time,to_time,to_date,from_date,product_type,start_price,reserve_price,increment_price,isActive ) async {
    _isLoading = true;
    notifyListeners();
    ...
    ApiResponse response = await restaurantRepo.addProduct(product,addProduct ,_fields, productImages, thumbnail, metaImage, token, isAdd, isActiveColor,from_time,to_time,to_date,from_date,product_type,start_price,reserve_price,increment_price,isActive,
    );
    if(response.response != null && response.response.statusCode == 200) {
