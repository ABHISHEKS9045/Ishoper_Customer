...
class ChatProvider extends ChangeNotifier {
  ...
  void sendMessage(MessageBody messageBody, BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      _chatList.add(ChatModel(sellerId: int.parse(messageBody.shopId), message: messageBody.message, sentByCustomer: 1, sentBySeller: 0, seenByCustomer: 0, seenBySeller: 1, shopId: int.parse(messageBody.shopId), createdAt: DateConverter.localDateToIsoString(DateTime.now())));
      notifyListeners();
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
