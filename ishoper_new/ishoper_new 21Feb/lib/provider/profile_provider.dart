...
class ProfileProvider extends ChangeNotifier {
  ...
  static const String TAG = "ProfileProvider";
  ...
  Future<void> initAddressList(BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
     ...
     // apiResponse.response.data.forEach((address) => _addressList.add(AddressModel.fromJson(address)));
      ...
      debugPrint("$TAG init Address List ===========> ${apiResponse.error.toString()}");
 ...
 var customerId;
  Future<String> getUserInfo(BuildContext context) async {
    ...
    // List  Customerdata = apiResponse.response.data ;
      ...
      debugPrint("$TAG get User Info ===========> ${apiResponse.error.toString()}");
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
    return userID;
  ...
  void initAddressTypeList(BuildContext context) async {
    if (_addressTypeList.length == 0) {
      ...
      if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
        ...
        debugPrint("$TAG init Address Type List ===========> ${apiResponse.error.toString()}");
        ApiChecker.checkApi(context, apiResponse);
      }
      notifyListeners();
    }
  ...
  Future addAddress(AddressModel addressModel, Function callback) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      debugPrint("$TAG add Address ===========> ${apiResponse.error.toString()}");
