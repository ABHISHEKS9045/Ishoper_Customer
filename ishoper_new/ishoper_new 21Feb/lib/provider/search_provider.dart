...
class SearchProvider with ChangeNotifier {
  ...
  void searchHintProduct(String query, BuildContext context) async {
    searchHintList = [];
    notifyListeners();

    ApiResponse apiResponse = await searchRepo.getSearchHintProductList(query);
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      if(apiResponse.response.data['total_size']==0)
        searchHintList = [];
      else{
        for(int i=0;i<apiResponse.response.data['total_size'];i++){
          searchHintList.add(apiResponse.response.data['products'][i]['name']);
        }
      }
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    print(searchHintList.length);
    notifyListeners();
  }
